...
class Build {
  ...
  getYAMLObject(svc, service_id, functions, stage) {
    let customOptions = svc.custom || {}
    let microOptions = customOptions.micro || {}
    let includePaths = microOptions.include || ['lib', 'node_modules']

    delete customOptions.micro
    customOptions.stage = '${opt:stage, self:provider.stage}'
    customOptions.prefix = '${self:service}-${self:provider.stage}'
    ...
    let payload = {
      ...
      plugins: svc.plugins,
      package: {
        include: includePaths.map(path => `../../${path}/**`)
      },
      functions: functions
    };
